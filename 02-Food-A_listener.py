"""
  This program listens for  messages when activated. 
    It is listening for messages from the producer_smoker.py program. It will recieve 
    messages which include the temperature of a food cooking in the smoker "02_Food-A". It will send an alert if the food has not warmed up by 
...
alert = "Warning! Food A is stalled and may not be warming up correctly, please check your food!"
...
def FoodA_callback(ch, method, properties, body):
    ...
    if len(FA_deque) == 20:
        ...
        if FAalert < 1:
            print(alert)
    print(f" [x] Received Food A temperature.  Food A temperature is {FoodA_message}")
